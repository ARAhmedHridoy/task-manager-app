...
class _CompletedTaskScreenState extends State<CompletedTaskScreen> {
  ...
  Widget _buildTasksSummary() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      if (_completedTaskController.taskCountByStatusModel == null) {
        _completedTaskController.getTaskCountingByStatus();
      }
    });
    ...
    return SingleChildScrollView(
      child: GetBuilder<CompletedTaskController>(builder: (controller) {
        final taskMap = {
          for (var task in _completedTaskController
  ...
  Future<void> _getTaskCountingByStatus() async {
    final bool isSuccess =
        await _completedTaskController.getTaskCountingByStatus();

    if (!isSuccess) {
      Get.snackbar('Error', _completedTaskController.errorMessage!);
    }
  }

  Future<void> _getNewTaskList() async {
    final bool isSuccess = await _completedTaskController.getNewTaskList();
  ...
  Future<void> _deleteTask(String taskId) async {
    final bool isSuccess = await _completedTaskController.deleteTask(taskId);
  ...
  Future<void> _updateTaskStatus(String taskId, String newStatus) async {
    final bool isSuccess = await _completedTaskController.updateTaskStatus(
      taskId,
      newStatus,
    );
